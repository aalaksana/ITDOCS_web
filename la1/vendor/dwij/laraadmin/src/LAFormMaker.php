		...
		if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "write")) {
			...
			switch ($field_type->name) {
				...
				case 'Dropdown':
					...
					$user = Auth::user()->id;
					$role= DB::select("SELECT r.id from roles r, users u, role_user ru 
					WHERE r.id=ru.role_id and ru.user_id=u.id and u.id=".$user);

						if($module->name=='Dokumens' && $field_name=='jenis')
						{
								
							$data = DB::select("Select jenis from doctypes where role='".$role[0]->id.
												"' or (role=role and '".$role[0]->id."'='1')");
							$data = array_map(function($object){
							    return (array) $object;
							}, $data);
							$data=array_column($data, 'jenis');
							$popup_vals=$data;
